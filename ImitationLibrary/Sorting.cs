...
namespace ImitationLibrary
{
    ...
    {
        public static void MergeSort<T, I>(T[] Arr, Func<T, I> Value, int Left, int Right) where I : IComparable<I>
        {
            ...
            {
                if (Value(Arr[Left]).CompareTo(Value(Arr[Right]))==1)//Swap the 2 values if they are the wrong way round
                {
                    Swap(Arr, Left, Right);
                }
            }
        ...
        private static void Merge<T, I>(T[] Arr, int Left, T[][] SetsToMerge, Func<T, I> Value) where I : IComparable<I>
        {
            ...
            for (int i = Left, iL = 0, iR = 0; iL < LeftLength || iR < RightLength; i++)//Iterate until we have merged all values on both regions
            {
                ...
                else if (Value(SetsToMerge[0][iL]).CompareTo(Value(SetsToMerge[1][iR]))==1)
